...
router.post('/', requireAuth, async (req: AuthedRequest, res) => {
  try {
    ...
    if (lines.length === 0) {
      return res
        .status(400)
        .json({ error: 'At least one spray foam line is required.' })
    }
    const normalizedLines = lines
      .map((line, index) => ({
        ...
        squareFeet: cleanDecimal(line.squareFeet),
        thicknessInches: cleanDecimal(line.thicknessInches),
        boardFeet: cleanDecimal(line.boardFeet),
